...
function drawUserTable(userInfos) {
    ...
    $.each(userInfos, function(id, userData)
    {
        ...
        var userTableDiv = '<div class="userlistcelldiv">' +
                                ...
                                '<div class="optdiv">' +
                                    '<button id="accept' + userID + '" class="btn btn-primary request-accept-btn">Accept</button>' +
                                    '<button id="reject' + userID + '" class="btn btn-danger request-reject-btn">Reject</button>' +
                                '</div>' +
                            '</div>';
        if(i < userCount)
        {
    ...
    $(".request-accept-btn").click(function(e) {
        var clickedUserID = 0;
        var btnID = $(this).attr("id");
        clickedUserID = btnID.replace("accept", "");

        alert('accept ' + clickedUserID );
    });

    $(".request-reject-btn").click(function(e) {
        var clickedUserID = 0;
        var btnID = $(this).attr("id");
        clickedUserID = btnID.replace("reject", "");

        alert('reject ' + clickedUserID );
    });
